...
const workerUrl = (): URL =>
	new URL(import.meta.url.endsWith('.ts') ? './combat-worker.ts' : './combat-worker.mjs', import.meta.url);
...
export class CombatVerifierPool {
	...
	constructor(
		...
		if (!Number.isInteger(workerCount) || workerCount < 1) {
			throw new RangeError('Combat worker count must be positive');
		}
		if (!Number.isInteger(queueLimit) || queueLimit < 1) {
			throw new RangeError('Combat queue limit must be positive');
		}
	...
	async close(): Promise<void> {
		...
		for (const task of queued) {
			task.complete({
				...
				error: 'Combat verifier closed before verification',
			});
		}
	...
	private completeWorkerTask(index: number, slot: WorkerSlot, message: CombatWorkerResponse): void {
		if (this.slots[index] !== slot || slot.current?.id !== message.taskId) {
			return;
		}
