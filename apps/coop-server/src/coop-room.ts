...
type LeakValidation =
	...
	| {
			ok: false;
			reason:
				'invalid-ordinal' | 'unknown-signal' | 'unknown-variant' | 'unknown-entrance' | 'signal-not-in-phase';
			leakIndex: number;
...
export class CoopRoom {
	...
	join(name: string, socket: WebSocket): RoomPlayer | null {
		if (this.closed || this.phase !== 'lobby' || this.players.p2) {
			return null;
		}
	...
	disconnect(playerId: CoopPlayerId, socket: WebSocket): void {
		...
		player.reconnectTimer = setTimeout(() => {
			if (player.connected || this.closed) {
				return;
			}
	...
	leave(playerId: CoopPlayerId, socket: WebSocket): void {
		const player = this.players[playerId];
		if (!player || player.socket !== socket || this.closed) {
			return;
		}
	...
	handle(
		...
		if (message.type === 'plan-command') {
			this.handlePlanCommand(player, message, traceId);
		} else if (message.type === 'transfer-shards') {
			this.handleTransfer(player, message, traceId);
		} else if (message.type === 'set-ready') {
			this.handleReady(player, message, traceId);
		} else if (message.type === 'draft-decision') {
			this.handleDraft(player, message, traceId);
		} else {
			this.handleCombatResult(player, message.result, traceId);
		}
	}
	...
	private handlePlanCommand(
		player: RoomPlayer,
		message: Extract<CoopClientMessage, { type: 'plan-command' }>,
		traceId: string,
	): void {
		if (!this.canPlan(player)) {
			return this.reject(player, traceId, 'planning-locked');
		}
		const result = applyCoopPlanningCommand(player.plan, message.command, this.levelId, player.towerSeed);
		if (!result.ok) {
			return this.reject(player, traceId, result.reason);
		}
	...
	private handleTransfer(
		player: RoomPlayer,
		message: Extract<CoopClientMessage, { type: 'transfer-shards' }>,
		traceId: string,
	): void {
		if (!this.canPlan(player)) {
			return this.reject(player, traceId, 'planning-locked');
		...
		if (!peer || peer.eliminated || peer.ready) {
			return this.reject(player, traceId, 'recipient-unavailable');
		}
		if (player.plan.shards < message.amount) {
			return this.reject(player, traceId, 'insufficient-shards');
		}
		...
		for (const participant of this.listPlayers()) {
			send(participant.socket, notification);
		}
	...
	private handleReady(
		player: RoomPlayer,
		message: Extract<CoopClientMessage, { type: 'set-ready' }>,
		traceId: string,
	): void {
		...
		if (
			this.players.p2 &&
			active.length > 0 &&
			active.every((candidate) => candidate.ready && candidate.connected)
		) {
			if (this.phase === 'lobby') {
				this.beginDraft(getLevel(this.levelId).moduleDraft.initialPicks);
			} else {
				this.beginLocalDefense();
			}
		}
	}
	...
	private handleDraft(
		player: RoomPlayer,
		message: Extract<CoopClientMessage, { type: 'draft-decision' }>,
		traceId: string,
	): void {
		if (
			this.phase !== 'draft' ||
		...
		this.bumpAndBroadcast();
		const active = this.activePlayers();
		if (!active.every((candidate) => candidate.draftLocked)) {
			return;
		}
	...
	private handleCombatResult(player: RoomPlayer, result: CoopCombatResult, traceId: string): void {
		...
		const verification: CombatVerificationRequest = {
			...
			plan: structuredClone(player.plan),
			signals:
				this.phase === 'reinforcement' && this.reinforcement ? structuredClone(this.reinforcement.signals) : [],
		};
	...
	private finishCombatVerification(
		...
		if (this.closed || !player.combatVerifying) {
			return;
		}
		player.combatVerifying = false;
		if (this.phaseId !== phaseId || player.combatSubmitted) {
			return;
		}
		...
		if (this.phase === 'reinforcement') {
			...
			if (owner) {
				this.applyLeaks(owner, acceptedResult.leaks);
			}
			this.finishWave();
			return;
		}
		const active = this.activePlayers();
		if (!active.every((candidate) => this.combatResults[candidate.id])) {
			return;
		}
		if (active.length === 2) {
			...
			if (firstLeaks.length > 0 && secondLeaks.length === 0) {
				return this.beginReinforcement(second, first, firstLeaks);
			}
			if (secondLeaks.length > 0 && firstLeaks.length === 0) {
				return this.beginReinforcement(first, second, secondLeaks);
			}
			if (firstLeaks.length > 0 && secondLeaks.length > 0) {
			...
			const only = active[0];
			if (only) {
				this.applyLeaks(only, this.combatResults[only.id]?.leaks ?? []);
			}
		}
		this.finishWave();
	}
	...
	private beginDraft(totalPicks: number): void {
		this.phase = 'draft';
		this.draftRuntime = createCoopDraftRuntime(
			(this.roomSeed ^ Math.imul(this.wave + 1, 0x9e3779b9)) >>> 0,
			totalPicks,
		);
		this.draftRuntime.players = this.draftPlayers;
		for (const state of Object.values(this.draftPlayers)) {
			state.previousChoices.clear();
		}
	...
	private beginLocalDefense(): void {
		...
		for (const id of COOP_PLAYER_IDS) {
			delete this.combatResults[id];
		}
		for (const player of this.listPlayers()) {
			player.ready = false;
	...
	private finishWave(): void {
		...
		const bonus = Math.round(
			(ECONOMY_BALANCE.waveBonusBase + this.wave * ECONOMY_BALANCE.waveBonusPerWave) * difficulty.economy,
		);
		for (const player of active) {
	...
	private validateLeaks(leaks: readonly CoopLeakedSignal[]): LeakValidation {
		...
		for (const [index, leak] of leaks.entries()) {
			if (leak.ordinal !== index) {
				return { ok: false, reason: 'invalid-ordinal', leakIndex: index, leak };
			}
			if (!signalIds.includes(leak.type)) {
				return { ok: false, reason: 'unknown-signal', leakIndex: index, leak };
			}
			if (
	...
	private resendActivePhase(player: RoomPlayer): void {
		if (this.phase === 'local-defense' && !player.eliminated && !player.combatSubmitted) {
			...
			this.phase === 'reinforcement' &&
			this.reinforcement?.defenderId === player.id &&
