...
const [server, worker] = await Promise.all([
	...
	esbuild.build({
		...shared,
		entryPoints: ['src/combat-worker.ts'],
		outfile: 'dist/combat-worker.mjs',
		metafile: true,
	}),
]);
...
const forbiddenInputs = inputs.filter((path) =>
	/packages[\\/]web-shared|apps[\\/]web-(?:single|coop)|\.(?:tsx|css)$/.test(path),
);
if (forbiddenInputs.length > 0) {
	throw new Error(`Server bundle includes browser inputs:\n${forbiddenInputs.join('\n')}`);
}
const mainThreadSimulation = Object.keys(server.metafile.inputs).filter((path) =>
	/game[\\/]engine\.ts$|packages[\\/]coop[\\/]src[\\/]simulation\.ts$/.test(path),
);
if (mainThreadSimulation.length > 0) {
	throw new Error(`Server main thread includes combat simulation:\n${mainThreadSimulation.join('\n')}`);
}
const output = `${await readFile('dist/server.mjs', 'utf8')}\n${await readFile('dist/combat-worker.mjs', 'utf8')}`;
if (/react-i18next|ReactDOM|coop\.entryAction/.test(output)) {
	throw new Error('Server bundle contains browser runtime content');
}
