...
export function FullSiteApp() {
	...
	useEffect(() => {
		...
		const warm = (): void => {
			void loadCoopFeature().catch(() => undefined);
		};
		const timeout = window.setTimeout(warm, 1_500);
		const idle = 'requestIdleCallback' in window ? window.requestIdleCallback(warm, { timeout: 1_500 }) : null;
		return () => {
					...
					<button type="button" className="coop-home-entry" onClick={enterCoop}>
						<svg className="coop-home-entry-mark" viewBox="0 0 44 30" aria-hidden="true">
							...
							<path
								...
								d="m10 3 5 5-5 5-5-5z"
							/>
							<path
								className="coop-home-entry-player coop-home-entry-player-left"
								d="m3 27v-6l3-9h8l3 9v6z"
							/>
							<path
								className="coop-home-entry-player coop-home-entry-player-right"
								d="m34 3 5 5-5 5-5-5z"
							/>
							<path
								className="coop-home-entry-player coop-home-entry-player-right"
								d="m27 27v-6l3-9h8l3 9v6z"
							/>
						</svg>
	...
	if (Feature) {
		return <Feature onExit={exitCoop} />;
	}
	if (loadFailed) {
		return (
			<main>
				<p role="alert">{t('coop.loadFailed')}</p>
				<button
					onClick={() => {
						setLoadFailed(false);
						void loadCoopFeature()
				...
				<button onClick={exitCoop}>{t('coop.singlePlayer')}</button>
			</main>
		);
	}
