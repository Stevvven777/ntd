...
const configuredServers = typeof __PRISM_BASTION_COOP_SERVERS__ === 'undefined' ? {} : __PRISM_BASTION_COOP_SERVERS__;
const allowServerOverride =
	typeof __PRISM_BASTION_COOP_ALLOW_SERVER_OVERRIDE__ !== 'undefined' && __PRISM_BASTION_COOP_ALLOW_SERVER_OVERRIDE__;
...
export function resolveCoopServerUrl(
	...
	if (selection) {
		...
		if (allowArbitraryUrl && isWebSocketUrl(selection)) {
			return selection;
		}
	}
