...
import {
	...
	moduleName,
	rarityLabel,
} from '@prism-bastion/web-shared/i18n/presentation';
...
export function CoopDraftScreen({ room, playerId, onChoose, onOpenThought }: CoopDraftScreenProps) {
	...
	return (
		<main className={styles.stageShell}>
			<section className={`${styles.stageConsole} ${styles.draftConsole}`}>
				<header className={styles.draftMasthead}>
					<div>
						<p className={styles.eyebrow}>{t('coop.phase.draft')}</p>
						<h1>{t('coop.draftTitle')}</h1>
					</div>
					<div className={styles.draftHeadActions}>
						{progressOffer ? (
							<DraftProgress
								className={styles.coopProgress}
								current={progressOffer.pick}
								total={progressOffer.totalPicks}
							/>
						) : null}
						<button
				...
				<div className={styles.draftWorkspace}>
					{peer ? (
						<aside className={styles.peerOffer}>
							<div className={styles.peerIdentity}>
								<p className={styles.offerRole}>{t('coop.friendOffer')}</p>
								<h2>{peer.name}</h2>
							</div>
							<div className={styles.peerChoices}>
								{peer.draftOffer?.choices.map((moduleId: ModuleId) => {
									...
									return (
										<details
											...
											data-dimmed={peer.draftLocked && !selected}
											style={moduleVariableStyle(definition)}
											key={moduleId}
										>
											<summary aria-label={moduleName(t, definition.id)}>
												<Icon />
												<span>{moduleName(t, definition.id)}</span>
											</summary>
											<div className={styles.peerPopover}>
												<strong>{moduleName(t, definition.id)}</strong>
												<small>
													{rarityLabel(t, definition.meta.rarity)} ·{' '}
													{kindLabel(t, definition.kind)}
												</small>
							...
							<div className={styles.peerStatus} data-locked={peer.draftLocked}>
								{peer.draftLocked ? t('coop.peerLocked') : t('coop.peerChoosing')}
							</div>
						</aside>
					) : null}
					{self ? (
						<section className={styles.localOffer}>
							<h2>
								{self.name} · {t('coop.you')}
							</h2>
							{self.draftLocked ? (
								<p className={styles.lockedNotice}>{t('coop.waitingForPeer')}</p>
							) : null}
							<div className={styles.choiceGrid}>
								{self.draftOffer?.choices.map((moduleId: ModuleId) => {
									const definition = coopModules.require(moduleId);
									...
									return (
										<ModuleDraftCard
											...
											readouts={[
												...
												{
													label: t('reward.inventory'),
													value: self.plan.inventory[moduleId] ?? 0,
												},
												{
													label: t('coop.poolLeft'),
													value: poolCount === 'unlimited' ? '∞' : poolCount,
												},
											]}
