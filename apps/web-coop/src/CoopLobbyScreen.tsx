...
export function CoopLobbyScreen({ room, playerId, self, peer, error, onLeave, onSetReady }: CoopLobbyScreenProps) {
	...
	return (
		<main className={styles.stageShell}>
			<section className={styles.stageConsole}>
				<header className={styles.roomMasthead}>
					...
					<div className={styles.stageHeading}>
						<p className={styles.eyebrow}>{t('coop.phase.lobby')}</p>
						<h2>{t('coop.lobbyTitle')}</h2>
						<span>{t('coop.shareCode')}</span>
					</div>
				...
				<div className={styles.endpointDeck}>
					...
					{lobbyPlayers.map((player) =>
						player ? (
							<section
								className={`${styles.endpoint} ${player.id === playerId ? styles.localEndpoint : styles.peerEndpoint}`}
								key={player.id}
							>
								<p className={styles.endpointLabel}>
									{player.id === playerId ? t('coop.you') : t('coop.friend')}
								</p>
								<h3>{player.name}</h3>
								<div className={styles.endpointState}>
									<span>{player.connected ? t('coop.connected') : t('coop.reconnecting')}</span>
									<b data-ready={player.ready}>
										{player.ready ? t('coop.ready') : t('coop.notReady')}
									</b>
								</div>
							</section>
						) : (
							<section className={`${styles.endpoint} ${styles.emptyEndpoint}`} key="empty">
				...
				<footer className={styles.readyDock}>
					<div>
						<strong>{peer ? t('coop.syncLinked') : t('coop.syncWaiting')}</strong>
						<span>{t('coop.readyHint')}</span>
					</div>
					<button
						disabled={room.players.length < 2 || !self.connected}
