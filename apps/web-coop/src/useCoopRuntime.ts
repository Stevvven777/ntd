...
export function useCoopRuntime(): CoopRuntime {
	...
	if (!clientRef.current) {
		clientRef.current = new CoopClient();
	}
				...
				(message: CoopServerMessage) => {
					if (message.type === 'session') {
						...
						for (const player of message.room.players) {
							installEngine(message.room, player.id);
						}
						setError(null);
						return;
					}
					...
					if (message.type === 'shards-transferred') {
						...
						setNotificationToast({
							message:
								message.fromId === currentPlayerId
									? t('coop.shardsSent', {
											amount: message.amount,
											name: counterpart?.name ?? t('coop.friend'),
										})
									: t('coop.shardsReceived', {
											amount: message.amount,
											name: counterpart?.name ?? t('coop.friend'),
										}),
					...
					if (message.type === 'rejected') {
						...
						setNotificationToast({
							message: t(`coop.error.${message.reason}`, {
								defaultValue: t('coop.error.commandRejected'),
							}),
							tone: 'warn',
							nonce: Date.now(),
						});
