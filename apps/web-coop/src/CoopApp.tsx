...
export function CoopApp({ onExit }: { onExit: () => void }) {
	...
	const openThought = (id: string): void => {
		...
		if (runtime.viewedPlayerId) {
			runtime.engines[runtime.viewedPlayerId]?.setAutoPauseCondition('thought-index', true);
		}
		setThoughtOpen({ id });
	};
	...
	const closeThought = (): void => {
		if (runtime.viewedPlayerId) {
			runtime.engines[runtime.viewedPlayerId]?.setAutoPauseCondition('thought-index', false);
		}
		setThoughtOpen(null);
	...
	const withThought = (content: ReactNode): ReactNode => (
		<>
			...
			{thoughtOpen ? (
				<ThoughtIndex initialThoughtId={thoughtOpen.id} onBack={closeThought} backToBattlefield />
			) : null}
		</>
	);
	...
	if (room.phase === 'draft') {
		return withThought(
			<CoopDraftScreen
				...
				playerId={playerId}
				onChoose={(choice) =>
					runtime.sendCommand({ type: 'draft-decision', expectedRevision: revision, choice })
				}
	...
	return withThought(
		<CoopBattleScreen
			...
			onOpenThought={openThought}
			onSetReady={(ready) => runtime.sendCommand({ type: 'set-ready', expectedRevision: revision, ready })}
			onTransferShards={(amount) =>
				runtime.sendCommand({ type: 'transfer-shards', expectedRevision: revision, amount })
			}
