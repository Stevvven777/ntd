...
import type { CoopRoomSnapshot } from '@prism-bastion/coop/types';
...
export function CoopResultScreen({ room, onLeave }: CoopResultScreenProps) {
	...
	return (
		<main className={styles.stageShell}>
			<section
				className={`${styles.resultConsole} ${room.result === 'victory' ? styles.victory : styles.defeat}`}
			>
				<div className={`${styles.screenSettings} ${styles.resultSettings}`}>
					<SettingsPanel />
				</div>
				<div className={styles.resultCopy}>
					<div className={styles.resultText}>
						<p className={styles.eyebrow}>{t('coop.result')}</p>
						<h1>{t(`coop.result.${room.result ?? 'defeat'}`)}</h1>
						<span>
							{t('coop.room')} <b className={styles.code}>{room.code}</b>
						</span>
					</div>
