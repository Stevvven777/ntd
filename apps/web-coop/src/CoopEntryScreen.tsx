				...
				<section className={styles.entryConsole}>
					...
					<div className={styles.entryOperations}>
								...
								>
									{DIFFICULTIES.map((difficulty) => (
										<option key={difficulty.id} value={difficulty.id}>
											{t(`difficulties.${difficulty.id}.name`)}
										</option>
									))}
								</select>
							</label>
							<button disabled={connection === 'connecting'} onClick={createRoom}>
								{t('coop.createAction')}
							</button>
						</section>
						<section className={styles.joinPanel}>
							...
							<button
								disabled={code.trim().length !== 6 || connection === 'connecting'}
								onClick={joinRoom}
							>
								{t('coop.joinAction')}
							</button>
						...
						<footer className={styles.entryFooter}>
							<p className={styles.status} data-state={connection}>
								<span aria-hidden="true" />
								{t(`coop.connection.${connection}`)}
							</p>
						</footer>
						{error ? (
