...
const javascriptFiles = (await readdir('dist', { recursive: true })).filter((path) => path.endsWith('.js'));
const javascript = (await Promise.all(javascriptFiles.map((path) => readFile(`dist/${path}`, 'utf8')))).join('\n');
if (!javascript.includes('coop.entryAction')) {
	throw new Error('Full client unexpectedly filtered co-op locale content');
}
const redirect = (coop) =>
	`<!doctype html><meta charset="utf-8"><script>const p=new URLSearchParams(location.search);${coop ? 'p.set("mode","coop")' : 'p.delete("mode")'};location.replace("./"+(p.size?"?"+p:"")+location.hash)</script>`;
await Promise.all([
