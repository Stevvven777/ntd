...
const forbiddenInputs = Object.keys(result.metafile.inputs).filter((path) =>
	/packages[\\/]coop|apps[\\/]web-coop|apps[\\/]coop-server/.test(path),
);
if (forbiddenInputs.length > 0) {
	throw new Error(`Single-player bundle includes co-op inputs:\n${forbiddenInputs.join('\n')}`);
}
const javascript = await readFile('dist/app.js', 'utf8');
if (javascript.includes('coop.')) {
	throw new Error('Single-player bundle contains co-op locale content');
}
await Promise.all([
