...
import type { DifficultyId, SignalVariantId, ModuleId } from '@prism-bastion/game-core/game/types';
import {
...
import { difficultyName, levelName, moduleName } from '@prism-bastion/web-shared/i18n/presentation';
...
import { SignalLedger, signalIconType, signalLabel } from '@prism-bastion/web-shared/ui/SignalLedger';
...
const ArchiveModule = ({ moduleId, count }: { moduleId: ModuleId; count?: number }) => {
	...
	return (
		<span className="archive-module" style={definition ? moduleVariableStyle(definition) : undefined}>
	...
	return (
		<div className="achievement-groups">
			{categories.map((category) => {
				...
				return (
					<section key={category} className="achievement-group" data-category={category}>
						<header>
...
const DefenseDetail = ({ record, onClose }: { record: DefenseRecord; onClose: () => void }) => {
	...
	return (
		<article className="defense-detail" data-result={record.result}>
			<header>
				<div>
			...
			<div className="defense-result-line">
			...
			<section>
				...
				<div className="defense-archive-table-scroll">
							...
							{record.waves.flatMap((wave) =>
								Object.entries(wave.signals).map(([variant, tally]) => (
									<tr key={`${wave.wave}-${variant}`}>
										<td>{wave.wave}</td>
										<th>
											<span className="defense-wave-signal">
								...
								{tower.slots.map((moduleId, slot) => (
									<li key={slot}>
										...
										{moduleId ? (
											...
											<span className="archive-module-empty">
...
const History = ({ records }: { records: DefenseRecord[] }) => {
	...
	const setFilter = (setter: (value: never) => void, value: string): void => {
			...
			<section className="history-list">
				<div className="history-filters" aria-label={t('defenseArchive.filters')}>
					<label>
						...
						<select value={result} onChange={(event) => setFilter(setResult, event.currentTarget.value)}>
					...
					<label>
						...
						<select value={levelId} onChange={(event) => setFilter(setLevelId, event.currentTarget.value)}>
							<option value="all">{t('defenseArchive.filter.all')}</option>
							{LEVELS.map((level) => (
					...
					<label>
						...
						<select
							value={difficultyId}
							onChange={(event) => setFilter(setDifficultyId, event.currentTarget.value)}
						>
							<option value="all">{t('defenseArchive.filter.all')}</option>
				...
				{visible.length === 0 ? (
					<div className="defense-archive-empty compact">
						<strong>{t('defenseArchive.noMatches')}</strong>
						<p>{t('defenseArchive.noMatchesDetail')}</p>
					</div>
				) : (
					<div className="defense-list" onKeyDown={navigateSelectionList}>
						{visible.map((record) => (
							<button
								key={record.id}
				...
				<footer className="history-pages">
					...
					<button disabled={page + 1 >= pageCount} onClick={() => setPage((value) => value + 1)}>
						→
					</button>
				</footer>
			</section>
			{selected ? (
				<DefenseDetail record={selected} onClose={() => setSelectedId(null)} />
			) : (
...
export function DefenseArchive({ repository, onBack }: { repository: DefenseArchiveRepository; onBack: () => void }) {
	...
	const pageRef = usePageArrowNavigation((direction) =>
		navigatePageSelection(pageRef.current, '.defense-archive-tabs [role="tab"]', direction),
	);
	...
	const moveTabFocus = (event: KeyboardEvent<HTMLButtonElement>, index: number): void => {
		const nextIndex =
			event.key === 'Home'
				? 0
				: event.key === 'End'
					? ARCHIVE_TABS.length - 1
					: event.key === 'ArrowRight'
						? (index + 1) % ARCHIVE_TABS.length
						: event.key === 'ArrowLeft'
							? (index - 1 + ARCHIVE_TABS.length) % ARCHIVE_TABS.length
							: -1;
		if (nextIndex < 0) {
			return;
		}
	...
	return (
		<main ref={pageRef} tabIndex={-1} className="archive-shell defense-archive-shell">
			<div className="defense-archive-frame">
				<ArchiveHeader
					...
					onBack={onBack}
					decoration={
						<div className="defense-archive-mark" aria-hidden="true">
							<i />
							<i />
							<i />
						</div>
				...
				/>
				<nav className="defense-archive-tabs" role="tablist" aria-label={t('defenseArchive.sections')}>
					{ARCHIVE_TABS.map((item, index) => (
						<button
							key={item}
				...
				>
					{error ? (
						<div className="defense-archive-empty">
						...
						<div className="defense-archive-loading">
							<i />
							<span>{t('defenseArchive.loading')}</span>
						</div>
					) : snapshot.warningCount > 0 ? (
						<div className="defense-archive-warning">
							{t('defenseArchive.corruptWarning', { count: snapshot.warningCount })}
						</div>
					) : null}
					{!error && snapshot ? (
						tab === 'overview' ? (
							<Overview records={snapshot.defenses} />
						) : tab === 'sectors' ? (
							<SectorArchive records={snapshot.defenses} />
						) : tab === 'achievements' ? (
				...
				<footer className="defense-archive-footer">
