...
export function RewardDraft({
	...
	return (
		<section
			className={`reward-panel${advancedVisible ? ' advanced' : ''}`}
			ref={panelRef}
			role="region"
			aria-label={isInitialDraft ? t('reward.initialAria') : t('reward.waveAria')}
		>
			<header className="reward-head">
					...
					<div className="reward-heading-meta">
						...
						<p>
							{isInitialDraft
								...
								: t('reward.waveDescription')}
						</p>
					</div>
				</div>
				{advancedVisible ? (
					<div className="reward-debug-summary" aria-label={t('reward.diagnosticsAria')}>
						<code className="reward-advanced-inline">
							s={quality(diagnostics.inventoryAverage)} a={quality(diagnostics.qualityAnchor)} b=
							{quality(diagnostics.computedBaseline)} u=+{quality(diagnostics.appliedBoost)} q=
							{quality(diagnostics.computedQuality)} ({diagnostics.retryCount}/{diagnostics.maxRetry}{' '}
							{diagnostics.highestOfferedQuality}:{diagnostics.skippedHighestQuality ?? '-'}:
							{diagnostics.projectileDeficit}:{diagnostics.guaranteedPoolSize})
						</code>
					</div>
				) : null}
				<div className="reward-head-actions">
					...
					>
						{t('reward.skip', { count: draft.skipsRemaining })}
					</button>
				</div>
			</header>
				...
				{draft.choices.map((moduleId) => {
					...
					return (
						<ModuleDraftCard
							...
							readouts={[
								{
									label: t('reward.energy'),
									value: (
										<>
											{definition.meta.energy}
											<EnergyBolt />
										</>
									),
								},
							...
							debug={
								advancedVisible && weight ? (
									<code className="reward-card-debug">
										b={compactWeight(weight.base)} r={compactWeight(weight.recent)} o=
										{compactWeight(weight.ownership)} t={compactWeight(weight.trailCompatibility)}{' '}
										p={compactWeight(weight.projectileCompatibility)} d=
										{compactWeight(weight.dependencyCompatibility)} w={compactWeight(weight.weight)}
									</code>
								) : null
							}
						/>
