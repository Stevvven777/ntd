...
const sourceExtensions = new Set(['.ts', '.tsx', '.js']);
...
const violations = [];
const inspect = (directories: string[], rules: Rule[]) => {
	for (const directory of directories) {
		for (const path of filesBelow(directory)) {
			const source = readFileSync(join(root, path), 'utf8');
			for (const { pattern, message } of rules) {
				if (pattern.test(source)) {
					violations.push(`${relative(root, join(root, path))}: ${message}`);
				}
			}
		}
	}
...
inspect(
	...
	[
		{
			pattern: /(?:from|import\s*)\s*['"](?:react|react-dom|react-i18next|i18next|ws|@prism-bastion\/coop)/,
			message: 'deterministic core imports a browser, i18n, WebSocket, or co-op dependency',
		},
		{
			pattern: /\b(?:CanvasRenderingContext2D|HTMLCanvasElement|EffectEngine)\b/,
			message: 'deterministic core exposes a browser presentation type',
		},
		{ pattern: /\b(?:displayColor|tint)\b/, message: 'deterministic core contains browser presentation metadata' },
		{
			pattern: /\bmode\s*===?\s*['"]coop['"]|\btype\s+GameMode\s*=.*coop/,
			message: 'deterministic core contains a co-op mode branch',
		},
	],
);
inspect(
	['packages/web-shared/src', 'apps/web-single/src'],
	[
		{
			pattern: /(?:from|import\s*)\s*['"]@prism-bastion\/(?:coop|web-coop)(?:\/|['"])/,
			message: 'single/shared browser code imports the co-op implementation',
		},
		{ pattern: /(?:\bt|\bi18n\.t)\(\s*['"]coop\./, message: 'single/shared browser code references co-op copy' },
	],
);
inspect(
	['packages/web-shared/src/module-presentations'],
	[
		{
			pattern: /\b(?:compile|targetEffect|onCast|onHit|onTrail|onDeploy|onTrigger)\s*:/,
			message: 'module presentation contains a runtime compiler or combat hook',
		},
	],
);
inspect(
	['apps/coop-server/src'],
	[
		{
			pattern: /(?:from|import\s*)\s*['"](?:react|react-dom|react-i18next|i18next|@prism-bastion\/web-)/,
			message: 'server imports browser runtime code',
		},
...
for (const dependency of Object.keys(singleManifest.dependencies ?? {})) {
	if (
		dependency === '@prism-bastion/coop' ||
		dependency === '@prism-bastion/web-coop' ||
		dependency === 'ws' ||
		dependency === 'zod'
	) {
