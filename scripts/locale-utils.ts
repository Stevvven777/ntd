...
type LocaleEntries = Record<string, unknown>;
type Locale = {
	filename: string;
	path: string;
	displayPath: string;
...
type LocaleValidationOptions = {
	requireAlignment?: boolean;
	requireFormatting?: boolean;
};

const isRecord = (value: unknown): value is LocaleEntries =>
...
export const readLocales = (localeDirectory = defaultLocaleDirectory): Locale[] => {
...
export const findEnglishLocale = (locales: Locale[]): Locale => {
	const english = locales.find(({ filename }) => filename === 'en.json');
	if (!english) {
		throw new Error('Locale directory must contain en.json');
	}
	return english;
};

export const canonicalLocaleSource = (entries: LocaleEntries, referenceKeys: string[]): string => {
...
export const validateLocales = (
	locales: Locale[],
	{ requireAlignment = false, requireFormatting = false }: LocaleValidationOptions = {},
): string[] => {
	const errors: string[] = [];
	...
	for (const locale of locales) {
		...
		if (requireAlignment) {
			for (const key of referenceKeys) {
				if (!Object.hasOwn(locale.entries, key)) {
					errors.push(`${locale.displayPath}: missing ${key}`);
				}
			}
		}

		for (const key of referenceKeys) {
			if (!moduleTextKey.test(key)) {
				continue;
			}
...
export const formatLocaleDirectory = (
	localeDirectory = defaultLocaleDirectory,
): { keyCount: number; changedFiles: number } => {
	...
	for (const locale of locales) {
		const completedEntries = Object.fromEntries(
			referenceKeys.map((key) => [key, Object.hasOwn(locale.entries, key) ? locale.entries[key] : null]),
		);
		const source = canonicalLocaleSource(completedEntries, referenceKeys);
		if (source === locale.source) {
			continue;
		}
