	...
	for (let step = 0; step < maximumDirectorSteps(definition); step += 1) {
		...
		if (location !== previousLocation) {
			...
			if (cue?.transition?.signalOpacity === 0 && presentation.signalOpacity > 0) {
				const targetableCount = director.runtime.engine.signals.filter(
					(signal) =>
						!signal.dead &&
						director.runtime.engine.towers.some(
							(tower) =>
								Math.hypot(
...
const inspectResolutionStructure = (definition: ThoughtDefinition): readonly ChoreographyWarning[] => {
	...
	for (const beat of definition.beats) {
		for (const cue of beat.cues ?? []) {
			if (isResolutionWait(cue)) {
				unresolvedSignals = false;
			}
			...
			if (cue.actions?.some((action) => action.type === 'spawn-signal')) {
				unresolvedSignals = true;
			}
			if (cue.transition?.signalOpacity !== undefined) {
				effectiveOpacity = cue.transition.signalOpacity;
			}
		}
	}
...
const inspectEnding = (definition: ThoughtDefinition): readonly ChoreographyWarning[] => {
	...
	if (!finalBeat || finalBeat.cues?.some(isResolutionWait)) {
		return [];
	}
	return [
		{
	...
	const finishSessions = (): void => {
		for (const [towerIndex, presentations] of activeSessions) {
			...
			if (!previous) {
				...
				const startsImmediateRebuild =
					first.animateChanges && first.modules.length === 1 && first.modules[0] === final.modules[0];
		...
		for (const cue of beatCues) {
			...
			for (const action of cue.actions ?? []) {
				if (action.type === 'setup') {
					loadouts.set(0, action.slots);
				}
				if (action.type === 'setup-towers') {
					for (const loadout of action.loadouts) {
						loadouts.set(loadout.towerIndex, loadout.slots);
					}
				}
			}
...
const warnings = thoughtRegistry
	.list()
	.flatMap((definition) => [
