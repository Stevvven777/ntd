import {
	calculateDraftBalanceRows,
	calculateWaveBalanceRows,
	sampleTowerStatAverages,
} from '@prism-bastion/game-core/game/balance-analysis';
...
console.table(
	calculateWaveBalanceRows().map((row) => ({
		level: row.levelName,
		wave: row.wave,
		units: row.units,
		...
		income: row.income,
		entrances: Object.entries(row.entranceFlow)
...
console.table(
	calculateDraftBalanceRows().map((row) => ({
		level: row.levelName,
		batch: row.rewardBatch,
		sigma: row.inventoryAverage,
		anchor: row.anchor.toFixed(2),
		center: row.qualityCenter.toFixed(2),
		quality: Object.entries(row.qualityShares)
			.map(([rarity, share]) => `${rarity}:${Math.round(share * 100)}%`)
			.join(' · '),
		kinds: Object.entries(row.kindShares)
