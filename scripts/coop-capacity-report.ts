...
if (!level) {
	throw new Error('Missing triune-delta benchmark level');
}
...
const realisticPlan = (): CoopPlayerPlan => {
	...
	if (!base) {
		throw new Error('Benchmark plan requires a starting tower');
	}
...
const stressPlan = (): CoopPlayerPlan => {
	...
	plan.towers = level.towerPads.map((_, index) =>
		cloneTower(base, index + 1, index, ['barrage', 'double-fork', 'fork', 'seeker', 'needle'], true),
	);
	plan.nextTowerId = plan.towers.length + 1;
	return plan;
};
...
const runCombat = (createPlan: PlanFactory): CombatMeasurement => {
	...
	controller.engine.subscribe((event) => {
		if (event.type === 'combat-phase-completed') {
			leaks = event.result.leaks.length;
		}
	});
	...
	if (!completed || leaks < 0) {
		throw new Error('Benchmark combat did not complete');
	}
...
console.log(
	`Co-op authoritative simulation benchmark (${process.version}, ${level.id} wave ${level.waves.length}, extreme)`,
);
console.log(`Runs: ${runCount} measured + ${warmupCount} warm-up per scenario`);
for (const scenario of scenarios) {
	for (let index = 0; index < warmupCount; index += 1) {
		runCombat(scenario.createPlan);
	}
...
const collectGarbage = (): void => {
	if (!globalThis.gc) {
		throw new Error('Run the capacity report with --expose-gc');
	}
...
const roomMemory = retainedBytesPerItem(2_000, (index) => {
	...
	const room = new CoopRoom({
		...
		onClosed: () => {},
		verifyCombat: () => {
			throw new Error('Idle room benchmark must not start combat');
		},
	});
