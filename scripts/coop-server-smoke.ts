...
const workerResponse = new Promise<{ ok: boolean; result: { leaks: unknown[] } }>((resolve, reject) => {
	const timeout = setTimeout(() => reject(new Error('Co-op combat worker did not respond')), 5_000);
	combatWorker.once('message', (message: { ok: boolean; result: { leaks: unknown[] } }) => {
...
const connect = (url: string, origin: string): Promise<number> =>
...
try {
	...
	if (!healthResponse.ok || health.ok !== true) {
		throw new Error('Co-op health check failed');
	}

	const webSocketUrl = `ws://127.0.0.1:${listeningPort}`;
	const rejectedStatus = await connect(webSocketUrl, 'https://attacker.example');
	if (rejectedStatus === 101) {
		throw new Error('Co-op server accepted an unauthorized Origin');
	}
	const allowedStatus = await connect(webSocketUrl, allowedOrigin);
	if (allowedStatus !== 101) {
		throw new Error(`Co-op server rejected its configured Origin (${allowedStatus})`);
	}
...
if (output.includes('ExperimentalWarning')) {
	throw new Error(`Production server emitted an ExperimentalWarning:\n${output}`);
}
