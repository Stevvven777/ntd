...
棱镜堡垒是一款以模块编程为核心的几何风格塔防游戏。把弹射物、修正、逻辑、尾迹和触发器装入炮塔槽位，编译器会从左到右将排列顺序转换为攻击程序；同一批模块经过重排即可形成截然不同的打击结果。

**[在 itch.io 游玩](https://fang-erj.itch.io/ntd) · [在 GitHub Pages 游玩](https://szdytom.github.io/ntd/)**

## 游戏特色

- **模块化构筑：** 38 个内置模块覆盖穿透、分叉、制导、弹跃、范围伤害、持续伤害与嵌套触发。
- **顺序即规则：** 修正与逻辑模块影响右侧的下一枚弹射物，未闭合的投射块可以回到开头再读取一次。
- **两种模式：** 正式模式包含库存限制、开局选牌、波后奖励和经济成长；创造模式开放无限模块、无限晶片与自定义信号台。
- **五个区域与五档难度：** 每个区域拥有独立路径、部署节点、敌人倍率与波次配置。
- **差异化精英：** 可再生护盾、死亡分裂与局部冷却压制会被逐步引入并最终组合。
