...
describe('co-op shared draft', () => {
	...
	it('does not overbook finite copies across simultaneous offers', () => {
		...
		for (const moduleId of new Set(appearances)) {
			...
			if (available !== 'unlimited') {
				expect(count).toBeLessThanOrEqual(available ?? 0);
			}
		}
	});
	...
	it('resolves both decisions before the next pick and preserves skip boost state', () => {
		...
		if (!offers.p1 || !offers.p2 || !firstChoice) {
			return;
		}
		expect(resolveCoopDraftDecision(runtime, 'p1', offers.p1, null, plans.p1, pool)).toEqual({ ok: true });
		expect(resolveCoopDraftDecision(runtime, 'p2', offers.p2, offers.p2.choices[0]!, plans.p2, pool)).toEqual({
			ok: true,
		});
