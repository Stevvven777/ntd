...
describe('spatial targeting systems', () => {
	...
	it('fills reusable query buffers without retaining stale results', () => {
		...
		expect(index.collectAlongSegment({ x: 0, y: 0 }, { x: 90, y: 0 }, 12, result).map((item) => item.id)).toEqual([
			1, 2,
		]);
	});
	...
	it('matches a rebuilt reference index across movement, death, spawn, and removal', () => {
		...
		const moving = Array.from({ length: 36 }, (_, index) => signal(index + 100, ((index * 47) % 520) - 80, 100, 0));
		moving.forEach((item, index) => {
			item.position.y = ((index * 71) % 380) - 60;
		});
		...
		for (let step = 0; step < 160; step += 1) {
			...
			expect(sortedIds(incremental.withinRadius(center, radius))).toEqual(
				sortedIds(reference.withinRadius(center, radius)),
			);
			expect(incremental.countWithinRadius(center, radius)).toBe(reference.countWithinRadius(center, radius));
			expect(incremental.findNearestWithinRadius(center, radius)?.id).toBe(
				reference.findNearestWithinRadius(center, radius)?.id,
			);
			const end = { x: center.x + 130, y: center.y - 75 };
			expect(sortedIds(incremental.alongSegment(center, end, 24))).toEqual(
				sortedIds(reference.alongSegment(center, end, 24)),
			);
		}
	});
