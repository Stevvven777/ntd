...
import {
	...
	segmentConvexExitTime,
	segmentRegularPolygonHitTime,
} from '@prism-bastion/game-core/game/collision';
...
describe('swept collision', () => {
	it('detects a circle crossed entirely within one step', () => {
		const time = segmentCircleHitTime({ x: 0, y: 0 }, { x: 100, y: 0 }, { x: 50, y: 0 }, 8);
