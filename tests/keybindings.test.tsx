...
import { defaultKeybindings, getKeybindings, KEYBINDINGS_STORAGE_KEY, parseKeybindings, saveKeybindings } from '@prism-bastion/web-shared/ui/keybindings';
...
it('respects disabled launching and externally controlled pause and speed', () => {
  ...
  const { rerender } = render(<GameHeader engine={engine} snapshot={engine.getSnapshot()} onExit={() => undefined} onLaunch={launch} launchDisabled />);
  for (const key of [' ', 'g', 'n']) fireEvent.keyDown(window, { key });
  expect(launch).not.toHaveBeenCalled(); expect(pause).not.toHaveBeenCalled(); expect(speed).not.toHaveBeenCalled();
  rerender(<GameHeader engine={engine} snapshot={engine.getSnapshot()} onExit={() => undefined} onLaunch={launch} launchDisabled={false} />);
  fireEvent.keyDown(window, { key: 'n' });
...
it('switches icon tabs with keyboard navigation and cancels recording when leaving controls', async () => {
  ...
  expect(screen.getByRole('tabpanel').getAttribute('aria-labelledby')).toBe(screen.getByRole('tab', { name: 'Key bindings' }).id);
  await user.click(screen.getByRole('button', { name: 'Pause / resume', exact: true }));
