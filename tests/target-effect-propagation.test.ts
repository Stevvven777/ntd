...
const deployAt = (engine: GameEngine, shot: ShotBlueprint, pathDistance: number): Projectile =>
	addProjectile(engine, shot, engine.path.pointAtDistance(pathDistance).position, { x: 0, y: 0 }, null);
...
describe('target effect propagation', () => {
	it('applies Frost and Corrosive Spore to direct and splash targets', () => {
		...
		if (!direct || !splash) {
			throw new Error('Expected two signals');
		}
		const shot = engine.modules.compile(['frost', 'toxin', 'nova']).shots[0];
		if (!shot) {
			throw new Error('Expected a nova shot');
		}
	...
	it('releases every burning modifier once at the splash center', () => {
		...
		const shot = engine.modules.compile(['ember-coating', 'searing-sigil', 'starfire-matrix', 'nova']).shots[0];
		if (!shot) {
			throw new Error('Expected a burning nova shot');
		}
		const impactPosition = { ...direct.position };
		const effects = vi.spyOn(engine.visuals, 'spawnMany');
		fireAt(engine, shot, direct);
	...
	it('applies Frost and Corrosive Spore to every Arcbolt chain target', () => {
		...
		if (!direct || !firstChain || !secondChain) {
			throw new Error('Expected three signals');
		}
		const shot = engine.modules.compile(['frost', 'toxin', 'arcbolt']).shots[0];
		if (!shot) {
			throw new Error('Expected an Arcbolt shot');
		}
		const effects = vi.spyOn(engine.visuals, 'spawnMany');
		fireAt(engine, shot, direct);
	...
	it('stops Arcbolt from chaining after Focus Core consumes its chain capacity', () => {
		...
		if (!shot) {
			throw new Error('Expected a focused Arcbolt shot');
		}
		fireAt(engine, shot, direct);
	...
	it.each([
		...
		if (!payload?.static) {
			throw new Error(`Expected a ${staticModule} payload`);
		}
	...
	it.each([
		...
		const { engine, signals } = prepareEngine([220, 220]);
		const carrier = engine.modules.compile(['impact-trigger', 'pulse', statusModule, staticModule]).shots[0];
		const payload = carrier?.payload[0];
		if (!payload?.static) {
			throw new Error(`Expected a ${statusModule} static payload`);
		}
		deployAt(engine, payload, 220);
	...
	it('applies Frost to both signals attacked by Tesla Sentry', () => {
		...
		const payload = carrier?.payload[0];
		if (!payload?.static) {
			throw new Error('Expected a Tesla Sentry payload');
		}
		deployAt(engine, payload, 220);
	...
	it('plays static modifier releases once at each newly affected target', () => {
		...
		const frostCarrier = frostSetup.engine.modules.compile(['impact-trigger', 'pulse', 'frost', 'toxic-cloud'])
			.shots[0];
		const frostPayload = frostCarrier?.payload[0];
		if (!frostPayload?.static) {
			throw new Error('Expected a Frost cloud and signals');
		}
		...
		const toxinCarrier = toxinSetup.engine.modules.compile(['impact-trigger', 'pulse', 'toxin', 'tesla-node'])
			.shots[0];
		const toxinPayload = toxinCarrier?.payload[0];
		const toxinSignal = toxinSetup.signals[0];
		if (!toxinPayload?.static || !toxinSignal) {
			throw new Error('Expected a Corrosive Tesla and signal');
		}
