...
describe('balance report aggregation', () => {
	...
	it('produces one valid row per configured wave', () => {
		...
		for (const level of LEVELS) {
			...
			levelRows.forEach((row, index) => {
				...
				if (!wave) {
					throw new Error(`Expected ${level.id} wave ${index + 1}`);
				}
				...
				expect(
					Object.keys(row.entranceFlow).every((entrance) => level.graph.entrances.includes(entrance)),
				).toBe(true);
			});
		}
	});
	...
	it('reports finite draft shares for every configured anchor and inventory sample', () => {
		...
		expect(rows).toHaveLength(
			LEVELS.reduce((total, level) => total + level.moduleDraft.qualityAnchors.length * 3, 0),
		);
