...
const handle = (room: CoopRoom, playerId: CoopPlayerId, message: Parameters<CoopRoom['handle']>[1]): void => {
	...
	if (socket) {
		room.handle(playerId, message, socket);
	}
};

const ready = (room: CoopRoom, playerId: CoopPlayerId): void =>
...
const finishDraft = (room: CoopRoom): void => {
	while (room.phase === 'draft') {
		for (const playerId of ['p1', 'p2'] as const) {
			...
			if (!player || player.eliminated || player.draftLocked || !player.draftOffer) {
				continue;
			}
			handle(room, playerId, {
				type: 'draft-decision',
...
const combatResult = (room: CoopRoom, playerId: CoopPlayerId, leaks: CoopLeakedSignal[], shardsEarned = 0) => ({
...
describe('co-op room state machine', () => {
	...
	it('waits at each draft pick and routes one-sided leaks through reinforcement', () => {
		...
		handle(
			room,
			'p1',
			combatResult(room, 'p1', [{ ordinal: 0, type: 'spark', variantId: 'spark', entrance: 'white-prism:0' }]),
		);
		expect(room.phase).toBe('local-defense');
	...
	it('eliminates a core while freezing its resources and lets the survivor continue', () => {
		...
		handle(
			room,
			'p2',
			combatResult(room, 'p2', [{ ordinal: 0, type: 'spark', variantId: 'spark', entrance: 'white-prism:0' }]),
		);
	...
	it('submits only one authority check while a result is pending', () => {
		...
		const verifyCombat = vi.fn<VerifyCombat>((_request, _claimed, complete) => {
			finish = complete;
		});
