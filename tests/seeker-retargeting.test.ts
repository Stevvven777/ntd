...
import type { Signal, Point, Projectile } from '@prism-bastion/game-core/game/types';
...
const addSeekingNeedle = (engine: GameEngine, targetId: number, position: Point, velocity: Point): Projectile => {
	const shot = engine.modules.compile(['seeker', 'needle']).shots[0];
	if (!shot) {
		throw new Error('Expected seeker and needle to compile into a shot');
	}
...
describe('seeking projectile retargeting', () => {
	it('acquires another signal when its original target dies', () => {
		...
		original.dead = true;
		const projectile = addSeekingNeedle(engine, original.id, engine.path.pointAtDistance(20).position, {
			x: 0,
			y: -620,
		});
