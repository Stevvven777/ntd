...
const encodeRawBody = (bodyValues: readonly number[]): string => {
	...
	const bytes = Uint8Array.from([...body, checksum >>> 24, checksum >>> 16, checksum >>> 8, checksum]);
	let value = 0n;
	for (const byte of bytes) {
...
describe('orchestration token codec', () => {
	...
	it('rejects damaged, malformed, and non-canonical tokens', () => {
		...
		expect(decodeOrchestration(`${token.slice(0, -1)}${replacement}`)).toEqual({
			ok: false,
	...
	it('keeps the explicit wire codebook aligned with the complete registry', () => {
		...
		expect(new Set(ORCHESTRATION_MODULE_IDS)).toEqual(
			new Set(
