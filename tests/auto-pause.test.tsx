...
describe('automatic pause', () => {
	it('is enabled by default and persists when disabled in settings', async () => {
		...
		expect(within(autoPauseGroup).getByRole('button', { name: 'Enabled' }).getAttribute('aria-pressed')).toBe(
			'true',
		);
		...
		expect(within(restoredGroup).getByRole('button', { name: 'Disabled' }).getAttribute('aria-pressed')).toBe(
			'true',
		);
	});
