...
import type { EnergyRefundBudget, Signal, Projectile, ShotBlueprint } from '@prism-bastion/game-core/game/types';
...
const addProjectile = (
	...
	addTestProjectile(engine, shot, position, velocity, targetId, {
		...(energyRefundBudget ? { energyRefundBudget } : {}),
	});
...
describe('new module compilation', () => {
	...
	it('registers the advanced modules', () => {
		expect(
			['singularity', 'reclaim-circuit', 'focus-core', 'condense-core', 'double-fork'].map(
				(id) => registry.require(id).kind,
			),
...
describe('new module combat behavior', () => {
	it('refunds tower energy from actual health damage', () => {
		...
		if (!signal || !tower) {
			throw new Error('Expected a signal and tower');
		}
		...
		if (!shot) {
			throw new Error('Expected a reclaim pulse');
		}
		...
		for (let step = 0; step < 30 && tower.energy === 0; step += 1) {
	...
	it('pulls signals on both sides toward an armed singularity', () => {
		...
		const tower = engine.towers[0];
		if (!ahead || !behind || !tower) {
			throw new Error('Expected two signals and a tower');
		}
		...
		if (!singularity?.static) {
			throw new Error('Expected a singularity payload');
		}
		addProjectile(engine, singularity, engine.path.pointAtDistance(220).position, { x: 0, y: 0 }, ahead.id);
