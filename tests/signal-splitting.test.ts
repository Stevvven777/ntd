...
function addLethalProjectile(engine: GameEngine, signal: Signal): void {
	...
	if (!shot) {
		throw new Error('Expected pulse to compile into a shot');
	}
...
describe('splitting signals', () => {
	it('replaces a defeated fracture core with configured non-splitting copies', () => {
		...
		if (!split) {
			throw new Error('Expected split-on-death capability');
		}
		...
		if (!parent) {
			throw new Error('Expected a fracture core');
		}
		...
		for (let step = 0; step < splitDelaySteps - 1; step += 1) {
			engine.update(FIXED_SIMULATION_STEP);
		}
		expect(engine.signals.filter((signal) => signal.type === 'fracture' && !signal.dead)).toHaveLength(0);
		expect(engine.signals).toContain(parent);
		...
		expect(
			children.every((signal) => signal.reward === Math.round(definition.stats.reward * split.rewardScale)),
		).toBe(true);
		expect(
			children.every(
				(signal) =>
					signal.coreDamage === Math.max(1, Math.round(definition.stats.coreDamage * split.coreDamageScale)),
			),
		).toBe(true);
		expect(engine.status).toBe('wave');
		...
		if (!child) {
			throw new Error('Expected a split child');
		}
		siblings.forEach((signal) => {
			signal.dead = true;
		});
