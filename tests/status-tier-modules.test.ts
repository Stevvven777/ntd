...
describe('tiered damage status modules', () => {
	...
	it('compounds direct damage penalties while retaining all four status modules', () => {
		const shot = registry.compile(['ember-coating', 'toxin', 'searing-sigil', 'starfire-matrix', 'pulse']).shots[0];

		expect(shot).toMatchObject({ damage: 10, energyCost: 73 });
		expect(shot?.modules).toEqual(['ember-coating', 'toxin', 'searing-sigil', 'starfire-matrix', 'pulse']);
	});
	...
	it('applies all four independently stackable statuses to one target', () => {
		...
		const shot = engine.modules.compile(['ember-coating', 'toxin', 'searing-sigil', 'starfire-matrix', 'pulse'])
			.shots[0];
		if (!shot) {
			throw new Error('Expected a compiled status shot');
		}
	...
	it('emits distinct particles while burning statuses remain active', () => {
		...
		placeSignal(engine, signal, 200);
		const shot = engine.modules.compile(['ember-coating', 'searing-sigil', 'starfire-matrix', 'pulse']).shots[0];
		if (!shot) {
			throw new Error('Expected a compiled burning shot');
		}
		fireAt(engine, shot, signal);
	...
	it('delivers the complete legendary burn over eight ticks', () => {
		...
		if (!shot) {
			throw new Error('Expected a compiled Starfire shot');
		}
		fireAt(engine, shot, signal);
...
describe('Ember Scorch Field', () => {
	...
	it('burns targets, refreshes one field status, and publishes the static channel', () => {
		...
		const field = carrier?.payload[0];
		if (!field?.static) {
			throw new Error('Expected an Ember Scorch Field payload');
		}
