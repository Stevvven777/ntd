...
import { getSignalCapability, signalRegistry, updateSignalHealthRegeneration } from '@prism-bastion/game-core/signals';
...
describe('Solar Sigil regeneration elite', () => {
	...
	it('returns any surviving health state to full in one fixed simulation frame', () => {
		...
		if (!capability) {
			throw new Error('Expected health regeneration');
		}
	...
	it('applies regeneration through the shared enemy update path', () => {
		...
		if (!signal) {
			throw new Error('Expected a Solar Sigil');
		}
		signal.speed = 0;
