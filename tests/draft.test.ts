...
describe('module draft system', () => {
	...
	const weightFor = (moduleId: ModuleId, overrides: Partial<DraftRollOptions> = {}) => {
		const row = calculateModuleDraftWeights(options(overrides)).find(
			(candidate) => candidate.moduleId === moduleId,
		);
		if (!row) {
			throw new Error(`Expected draft weight for ${moduleId}`);
		}
		return row;
	};
	...
	it('returns four unique choices from one weighted module pool', () => {
		...
		expect(
			allProjectile.every((id) => definitions.find((definition) => definition.id === id)?.kind === 'projectile'),
		).toBe(true);
	});
	...
	it('applies the projectile and trail-carrier compatibility rules', () => {
		...
		).toBeCloseTo(DRAFT_BALANCE.projectileShortageMultiplier * DRAFT_BALANCE.noTrailCarrierMultiplier);
