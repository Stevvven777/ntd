...
describe('expiration and terrain trigger modules', () => {
	...
	it('waits for a piercing carrier final hit before releasing', () => {
		...
		if (!shot) {
			throw new Error('Expected an expiration carrier');
		}
		...
		for (let index = 0; index < signals.length; index += 1) {
			...
			if (!signal) {
				throw new Error('Expected a placed signal');
			}
	...
	it('releases when a Prism Crown shield fully absorbs the carrier', () => {
		...
		if (!crown) {
			throw new Error('Expected a Prism Crown');
		}
		placeSignal(expirationEngine, crown, 180);
		const expirationShot = expirationEngine.modules.compile(['expiration-trigger', 'pulse', 'proximity-mine'])
			.shots[0];
		if (!expirationShot) {
			throw new Error('Expected an expiration carrier');
		}
		...
		if (!impactShot) {
			throw new Error('Expected an impact carrier');
		}
	...
	it.each(['timer-trigger', 'terrain-trigger'] as const)(
		...
		(triggerId) => {
			...
			if (!shot) {
				throw new Error(`Expected a ${triggerId} carrier`);
			}
	...
	it('releases on the first tick after crossing the channel centerline', () => {
		...
		if (!shot) {
			throw new Error('Expected a terrain carrier');
		}
		const centerline = engine.path.pointAtDistance(200).position;
		const projectile = addProjectile(engine, shot, { x: centerline.x, y: centerline.y - 10 }, { x: 0, y: 1_200 });
	...
	it('releases on a collision before completing a centerline crossing', () => {
		...
		if (!signal) {
			throw new Error('Expected a collision target');
		}
