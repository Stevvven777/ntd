...
describe('Surge wave movement', () => {
	it('uses a smooth sine pulse while preserving its configured average speed', () => {
		...
		if (!movement) {
			throw new Error('Expected Surge wave movement');
		}
		...
		expect(restMultiplier + (movement.peakSpeedMultiplier - restMultiplier) * pulseMean).toBeCloseTo(1, 8);
		...
		expect(signalMovementSpeedMultiplier(movement, movement.cycle - 1e-8)).toBeCloseTo(
			signalMovementSpeedMultiplier(movement, 1e-8),
			8,
		);
		...
		if (!signal) {
			throw new Error('Expected a Surge signal');
		}
		...
		for (let step = 1; step < cycleSteps; step += 1) {
			engine.update(FIXED_SIMULATION_STEP);
		}
	...
	it('keeps tower interception based on average speed during a dash', () => {
		...
		if (!signal || !tower) {
			throw new Error('Expected a tower and Surge signal');
		}
		...
		if (!projectile) {
			throw new Error('Expected the tower to fire');
		}
		...
		if (!averagePrediction || !burstPrediction) {
			throw new Error('Expected both interception solutions');
		}
