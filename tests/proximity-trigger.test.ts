...
const createStaticProjectile = (engine: GameEngine, shot: ShotBlueprint, distance: number): Projectile => {
...
const proximityMineShot = (engine: GameEngine): ShotBlueprint => {
	const carrier = engine.modules.compile(['impact-trigger', 'pulse', 'proximity-mine']).shots[0];
	const mine = carrier?.payload[0];
	if (!mine?.static) {
		throw new Error('Expected a proximity mine payload');
	}
	return mine;
};
...
describe('static proximity detection', () => {
	...
	it('uses the configured center radius', () => {
		...
		if (!compiledShot.static) {
			throw new Error('Expected static shot configuration');
		}
