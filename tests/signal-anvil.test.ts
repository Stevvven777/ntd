...
import {
	...
	limitSignalHealthDamage,
} from '@prism-bastion/game-core/signals/capabilities/damage-cap';
...
describe('Prism Anvil layered armor elite', () => {
	it('caps only health damage above its configured threshold', () => {
		...
		if (!armor) {
			throw new Error('Expected damage-cap capability');
		}
	...
	it('applies the cap in the shared combat damage path', () => {
		...
		if (!shot || shot.damage <= cap) {
			throw new Error('Expected a projectile above the armor cap');
		}
