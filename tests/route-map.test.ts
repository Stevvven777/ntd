...
describe('route map model', () => {
	...
	it('rejects malformed trees and migrates a legacy polyline without changing it', () => {
		...
		const legacy = legacyPathToGraph(
			[
				{ x: 0, y: 0 },
				{ x: 3, y: 0 },
				{ x: 3, y: 4 },
...
describe('multi-entrance spawning and targeting', () => {
	...
	it('broadcasts all signal types unless a fixed entrance is configured', () => {
		...
		if (!entrance) {
			throw new Error('Expected an entrance fixture');
		}
	...
	it('spawns a deterministic wave across all entrances', () => {
		...
		if (broadcastWaveIndex < 0) {
			throw new Error('Expected a wave beginning with a broadcast entry');
		}
		...
		engine.startWave();
		for (let step = 0; step < 120 && engine.signals.length === 0; step += 1) {
			engine.update(1 / 60);
		}
	...
	it('moves and prioritizes signals by their own route distance to the core', () => {
		...
		if (!northEntrance || !southEntrance) {
			throw new Error('Expected at least two entrances');
		}
		...
		if (!north || !south || !tower) {
			throw new Error('Expected signals and a tower');
		}
		...
		expect(selectTowerTarget(tower, [south, north], undefined, (signal) => engine.distanceToCore(signal))?.id).toBe(
			north.id,
		);
		tower.targeting = 'core-farthest';
		expect(selectTowerTarget(tower, [north, south], undefined, (signal) => engine.distanceToCore(signal))?.id).toBe(
			south.id,
		);
	});
});
