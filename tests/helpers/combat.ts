...
export function advanceEngineUntil(engine: GameEngine, condition: () => boolean, seconds = 1): void {
	const steps = Math.ceil(seconds / FIXED_SIMULATION_STEP);
	for (let step = 0; step < steps && !condition(); step += 1) {
		engine.update(FIXED_SIMULATION_STEP);
	}
}
