		...
		level.waves.map((wave, waveIndex) => {
			...
			for (const entry of wave) {
				...
				for (const entrance of entrances) {
					entranceFlow[entrance] = (entranceFlow[entrance] ?? 0) + 1;
				}
				...
				if (split) {
					speedPressure +=
						((splitHp * splitCount * stats.speed * split.speedScale * level.signalSpeedScale) / 60) *
						copies;
				}
				killIncome += stats.reward * copies;
				if (split) {
					killIncome += Math.max(1, Math.round(stats.reward * split.rewardScale)) * splitCount * copies;
				}
				units += (1 + splitCount) * copies;
			}
			const spawnDuration =
				0.25 +
				wave.slice(0, -1).reduce((sum, entry) => sum + signalRegistry.require(entry.type).stats.spawnDelay, 0);
...
export function calculateDraftBalanceRows(): DraftBalanceRow[] {
	...
	return LEVELS.flatMap((level) =>
		level.moduleDraft.qualityAnchors.flatMap((anchor, rewardBatch) =>
			QUALITY_SAMPLES.map((inventoryAverage) => {
				...
				const qualityShares = Object.fromEntries(
					MODULE_RARITIES.map((rarity) => [
						rarity,
						shareFor((index) => definitions[index]?.meta.rarity === rarity),
					]),
				) as Record<ModuleRarity, number>;
				const kindShares = Object.fromEntries(
					MODULE_KINDS.map((kind) => [kind, shareFor((index) => definitions[index]?.kind === kind)]),
