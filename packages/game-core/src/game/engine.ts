...
export class GameEngine {
  ...
  synchronizeShards(shards: number): void {
    if (!this.externallyControlled || this.status === 'wave' || !Number.isFinite(shards)) return;
    this.shards = Math.max(0, Math.round(shards));
    this.emitState();
  }

  applyGamePlan(plan: GamePlan): void {
    if (!this.externallyControlled || this.status === 'wave') return;
  ...
  fastForwardCombatPhase(maxTicks = 120 * 900): boolean {
    if (!this.externallyControlled) return false;
    let ticks = 0;
  ...
  setTargeting(mode: TargetingMode): void {
    ...
    if (this.externallyControlled) {
      if (this.status !== 'planning' || !this.planningEnabled) return;
  ...
  placeTower(padIndex: number): void {
    ...
    if (this.shards < ECONOMY_BALANCE.towerCost) {
      this.emit({ type: 'notice', notice: { key: 'toast.buildNeeds', values: { cost: ECONOMY_BALANCE.towerCost }, tone: 'warn' } });
      return;
    }
    ...
    this.visuals.spawnMany(['game:tower-build-ring', 'game:tower-build-sparks'], { position: tower.position, color });
    this.emit({ type: 'notice', notice: {
      key: 'toast.built', values: { slots: tower.slots.length, energy: tower.maxEnergy }, tone: 'good',
    } });
    this.markConfigurationChanged();
    this.emitState();
  }
  ...
  installModule(slotIndex: number, moduleId: ModuleId | null): void {
    ...
    if (moduleId && !this.modules.get(moduleId)) {
      this.emit({ type: 'notice', notice: { key: 'toast.unknownModule', values: { module: moduleId }, tone: 'warn' } });
      return;
    }
  ...
  private highestDraftQuality(choices: readonly ModuleId[]): number {
    return choices.reduce((highest, moduleId) => Math.max(
      highest,
      MODULE_RARITIES[this.modules.require(moduleId).meta.rarity].qualityPoints,
    ), 1);
  }
  ...
  private checkWaveEnd(delta: number): void {
    ...
    if (this.externallyControlled) {
      this.status = 'planning';
      this.emit({ type: 'combat-phase-completed', result: {
