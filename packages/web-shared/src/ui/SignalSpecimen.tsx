...
import { getSignalCapability, signalRegistry, type SignalArchiveDemoMode } from '@prism-bastion/game-core/signals';
import {
...
export function SignalSpecimen({
	...
	useEffect(() => {
		...
		const spawnShieldEffect = (
			id:
				| typeof GAME_EFFECT_IDS.shieldHit
				| typeof GAME_EFFECT_IDS.shieldBreak
				| typeof GAME_EFFECT_IDS.shieldRestore,
		): void => {
			if (!shield) {
				return;
			}
		...
		const updateCrownShield = (delta: number): void => {
			...
			if (event === 'hit') {
				spawnShieldEffect(GAME_EFFECT_IDS.shieldHit);
			}
			if (event === 'break') {
				spawnShieldEffect(GAME_EFFECT_IDS.shieldBreak);
			}
			if (event === 'restore') {
				spawnShieldEffect(GAME_EFFECT_IDS.shieldRestore);
			}
		};

		const draw = (now: number): void => {
			const next = previewStateRef.current;
			if (next.type !== activeType || next.demoMode?.id !== activeDemoMode?.id) {
				resetPreviewState();
			}
			...
			if (cssWidth !== renderWidth || cssHeight !== renderHeight || deviceScale !== renderScale) {
				...
				if (pipeline) {
					pipeline.resize(cssWidth, cssHeight, deviceScale);
				} else {
			...
			const projectileProgress = shield ? archiveShieldProjectileProgress(shieldCycle) : null;
			const projectileVisible = projectileProgress === null ? 'false' : 'true';
			if (canvas.dataset.projectileVisible !== projectileVisible) {
				canvas.dataset.projectileVisible = projectileVisible;
			}
			...
			if (activeDemoMode?.specimen.kind === 'split-result' && split) {
				...
				if (showingSuppression) {
					sceneCtx.translate(SUPPRESSION_SOURCE.x, SUPPRESSION_SOURCE.y);
				}
			...
			if (pipeline) {
				...
				const splitActive =
					activeDemoMode?.specimen.kind === 'split-result' &&
					split !== undefined &&
