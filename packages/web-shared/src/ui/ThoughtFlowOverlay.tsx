...
const overlapArea = (
	...
	const overlapWidth = Math.max(
		0,
		Math.min(left + width, obstacleLeft + obstacle.width) - Math.max(left, obstacleLeft),
	);
	const overlapHeight = Math.max(
		0,
		Math.min(top + height, obstacleTop + obstacle.height) - Math.max(top, obstacleTop),
	);
	return overlapWidth * overlapHeight;
};

const closestPointOnRect = (target: OverlayPoint, element: HTMLElement): OverlayPoint => {
...
export function ThoughtFlowOverlay({
	...
	useLayoutEffect(() => {
		...
		const positionCallout = (): void => {
			...
			const overlapFor = (candidate: OverlayPosition): number =>
				obstacles.reduce(
					(sum, obstacle) =>
						sum +
						overlapArea(candidate.left, candidate.top, boundedWidth, boundedHeight, obstacle, rootBounds),
					0,
				);
		...
		const positionLoadoutDialog = (
			...
			const left = clamp(horizontal, margin, root.clientWidth - dialog.offsetWidth - margin);
			const top = clamp(vertical, margin, root.clientHeight - dialog.offsetHeight - margin);
	...
	return (
		<div ref={rootRef} className="thought-scene-overlay" data-cue={snapshot.cueId} style={cueStyle}>
			{snapshot.placementBurst ? (
				<div className="thought-placement-burst" style={placementBurstPosition ?? undefined} aria-hidden="true">
									...
									>
										{previousModule && PreviousIcon ? (
											<div
												...
												aria-hidden="true"
											>
												<span className="thought-loadout-module-icon">
													<PreviousIcon />
												</span>
			...
			{snapshot.loadoutMode === 'compact' || snapshot.loadoutMode === 'compact-leaving'
				? snapshot.loadoutTargets.map((target) => (
						<div
							ref={(element) =>
								registerOverlayRef(compactLoadoutRefs.current, target.towerIndex, element)
							}
			...
			{line ? (
				<svg
					...
					className="thought-overlay-line"
			...
			{snapshot.error ? (
				<section className="thought-scene-callout thought-scene-error" role="alert">
