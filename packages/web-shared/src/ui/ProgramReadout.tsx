...
export function ProgramReadout({
	...
	const capacityWarning =
		program.shots.length > 0 && program.energyCost > maxEnergy
			? t('program.capacity', { required: program.energyCost, capacity: maxEnergy })
			: null;
	const diagnostic =
		program.diagnostics.find((candidate) => candidate.severity === 'error') ?? program.diagnostics[0];
			...
			<div className={`program-readout ${warning ? 'warning' : ''}`}>
				...
				{diagnosticThought && onOpenThought ? (
					<button className="program-thought-link" onClick={() => onOpenThought(diagnosticThought.id)}>
						{t('thoughtIndex.explainDiagnostic')}
					</button>
				) : null}
			</div>
			{!hasTrigger ? null : (
				<div className="trigger-trace">
					<small>{t('program.payload')}</small>
					<div>
						{program.shots.map((shot, index) => (
							<TriggerNode key={`${shot.source}-${index}`} shot={shot} engine={engine} />
						))}
					</div>
				</div>
			)}
