								...
								{engine.rules.scenarioControls === 'creative' ? (
									<>
										<button
											...
											onClick={() => void importOrchestration()}
										...
										<button
											...
											onClick={() => void exportOrchestration()}
											disabled={transferPending !== null}
										>
											{t('workshop.export')}
										</button>
							...
							{tower.slots.map((moduleId, index) => (
								<ModuleSlot
									...
									onSelectModule={(id) => {
										...
										if (kindFilter !== 'all' && installed?.kind !== kindFilter) {
											setKindFilter('all');
										}
					...
					<section className="library-section">
						<div className="section-title library-title">
							<div>
								...
								<h3>
									{filterLabel} · {visibleDefinitions.length}
								</h3>
							</div>
						...
						<div className={`module-grid ${kindFilter === 'all' ? 'all-modules' : ''}`}>
							...
							{visibleDefinitions.length === 0 ? (
								<div className="module-library-empty">
									{t('workshop.emptyLibrary', { kind: filterLabel })}
								</div>
							) : null}
						</div>
					</section>
