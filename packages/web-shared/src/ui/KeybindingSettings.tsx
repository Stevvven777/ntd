  ...
  return <section className="settings-section settings-keybindings-section">
    ...
    <div className="keybinding-list">
      {keybindingActions.map((action) => <div className="keybinding-row" key={action}>
        ...
        <button type="button" aria-labelledby={`keybinding-${action}`} aria-pressed={recording === action}
          onBlur={() => { if (recording === action) setRecording(null); }}
          onClick={() => { setRecording(action); setMessage(''); }}>
          {recording === action ? t('settings.keys.listening') : bindings[action] ?? t('settings.keys.unbound')}
        </button>
      ...
      <button type="button" className="keybinding-reset" onClick={() => {
        saveKeybindings({ ...defaultKeybindings }); setRecording(null); setMessage('');
      }}>{t('settings.keys.reset')}</button>
    </div>
  </section>;
}
