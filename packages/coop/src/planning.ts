...
const mixSeed = (seed: number, ordinal: number): number =>
	(Math.imul((seed ^ 0x9e3779b9) >>> 0, 0x85ebca6b) + Math.imul(ordinal + 1, 0xc2b2ae35)) >>> 0;
...
export function createInitialCoopPlan(levelId: string, difficultyId: DifficultyId, seed: number): CoopPlayerPlan {
...
const installedCount = (
	plan: CoopPlayerPlan,
	moduleId: ModuleId,
	excluded?: { towerId: number; slot: number },
): number =>
	plan.towers.reduce(
		(sum, tower) =>
			sum +
			tower.slots.reduce(
				(count, installed, slot) =>
					count +
					Number(installed === moduleId && !(excluded?.towerId === tower.id && excluded.slot === slot)),
				0,
			),
		0,
	);
...
export function applyCoopPlanningCommand(
	...
	if (command.type === 'place-tower') {
		if (!Number.isInteger(command.padIndex) || !level.towerPads[command.padIndex]) {
			return { ok: false, reason: 'invalid-pad' };
		}
		if (plan.towers.some((tower) => tower.padIndex === command.padIndex)) {
			return { ok: false, reason: 'occupied-pad' };
		}
		if (plan.shards < ECONOMY_BALANCE.towerCost) {
	...
	if (command.type === 'set-targeting') {
		if (!isTargetingMode(command.targeting)) {
	...
	if (command.type === 'install-module') {
		...
		if (command.moduleId !== null) {
			if (!isKnownCoopModule(command.moduleId)) {
	...
	if (command.type === 'swap-modules') {
		if (
			...
			command.from < 0 ||
			command.to < 0 ||
			command.from >= tower.slots.length ||
			command.to >= tower.slots.length
		) {
			return { ok: false, reason: 'invalid-slot' };
		}
		[tower.slots[command.from], tower.slots[command.to]] = [
			tower.slots[command.to] ?? null,
			tower.slots[command.from] ?? null,
		];
		return { ok: true, plan };
	}
