import {
	...
	type GamePlanningCommand,
	type VisualFeedbackSink,
} from '@prism-bastion/game-core';
...
export class CoopGameController {
	...
	setPlanningEnabled(enabled: boolean): void {
		this.engine.setPlanningEnabled(enabled);
	}
	applyPlan(plan: CoopPlayerPlan): void {
		this.engine.applyGamePlan(plan);
	}
	synchronizeShards(shards: number): void {
		this.engine.synchronizeShards(shards);
	}
	startCombat(input: CombatPhaseInput): void {
		this.engine.startCombatPhase(input);
	}
	fastForward(maxTicks?: number): boolean {
		return this.engine.fastForwardCombatPhase(maxTicks);
	}
}
