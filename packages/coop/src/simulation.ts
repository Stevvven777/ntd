...
export function simulateAuthoritativeCombat(request: CombatVerificationRequest): CoopCombatResult {
	...
	engine.subscribe((event: GameEvent) => {
		if (event.type !== 'combat-phase-completed' || event.result.phaseId !== request.phaseId) {
			return;
		}
	...
	if (!result) {
		throw new Error(`Combat phase ${request.phaseId} completed without a result`);
	}
	return result;
}
...
export type CombatWorkerResponse =
	{ taskId: number; ok: true; result: CoopCombatResult } | { taskId: number; ok: false; error: string };
