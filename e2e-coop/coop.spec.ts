...
test('two friends join, draft simultaneously, and start local defense', async ({ browser }) => {
	...
	await expect(
		first.getByText('Next-wave signals').locator('..').getByRole('button', { name: 'Co-op console' }),
	).toBeVisible();
	...
	await coopConsoleButton.click();
	await expect(
		first.getByRole('dialog', { name: 'Bastion relay' }).getByRole('button', { name: 'Cancel ready' }),
	).toHaveCount(0);
	await first.getByRole('button', { name: 'Close co-op console' }).click();
